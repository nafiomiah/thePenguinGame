...
void draw_fish(int x, int y){
	...
	for (int j=x; j<=x+8; ++j){ // mouth
		plot_pixel(j, y+16, 0);
	}
	for (int j=x+31; j<=x+31+8; ++j){ // tail details
...
/* KEY base address */
	...
	*(KEY_ptr + 3) = press; // Clear the interrupt
	...
	if (press & 0x1){ // KEY0
		...
		if(penguinX>=260){   // Keep penguin from going out of right bound 
			penguinX=260;
		}
		...
		if(penguinX<=0){      // Keep penguin from going out of left bound
			penguinX=0;
		}
	
	}
	else if( press & 0x8){ // which is KEY3
		if (startGame == true){
